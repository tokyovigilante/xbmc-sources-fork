...
#include "All.h"
#include "resource.h"
#include "StdLibFileIO.h"//IO_HEADER_FILE
...
void __stdcall c_DeleteAPETag(IAPETag* tag)
{
  printf("Deleting tag: %p\n", tag);
  delete tag;
}
...
int __stdcall c_Tag_GetFieldString(void* tag, const str_utf16 * pFieldName, str_ansi * pBuffer, int * pBufferCharacters, BOOL bUTF8Encode)
{
	printf("Got tag: 0x%08lx\n", (void* )tag);
	return ((CAPETag* )tag)->GetFieldString(pFieldName, pBuffer, pBufferCharacters, bUTF8Encode);
}
