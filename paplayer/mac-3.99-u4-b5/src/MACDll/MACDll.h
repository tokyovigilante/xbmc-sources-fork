...
extern "C"
{
	...
	__declspec( dllexport ) IAPETag* __stdcall GetAPETag(const str_ansi * pFilename, bool bCheckID3Tag);
  	__declspec( dllexport ) long __stdcall GetAPEDuration(const str_ansi * pFilename);
	__declspec( dllexport ) int Tag_GetFieldString(void* tag, const str_utf16 * pFieldName, str_ansi * pBuffer, int * pBufferCharacters, BOOL bUTF8Encode);
}
...
extern "C"
{
	...
	__declspec( dllexport ) IAPETag* __stdcall c_GetAPETag(const str_ansi * pFilename, bool bCheckID3Tag);
	__declspec( dllexport ) void __stdcall c_DeleteAPETag(IAPETag* pTag);
  	__declspec( dllexport ) long __stdcall c_GetAPEDuration(const str_ansi * pFilename);
	__declspec( dllexport ) int c_Tag_GetFieldString(void* tag, const str_utf16 * pFieldName, str_ansi * pBuffer, int * pBufferCharacters, BOOL bUTF8Encode);
}
